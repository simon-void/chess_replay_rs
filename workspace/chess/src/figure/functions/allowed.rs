use crate::base::{BasicMove, PromotionType};
use crate::game::{Game, MoveResult, StoppedReason};

pub fn get_allowed_moves(game_config: &str) -> Vec<BasicMove> {
    let mut movable_moves: Vec<BasicMove> = vec!();
    ...
    game.get_reachable_moves().iter().filter(
        ...
        |&&a_move| {
            if let Some(promo_type) = a_move.promotion_type {
...
fn is_not_bound(game: &Game, a_move: BasicMove) -> bool {
...
mod tests {
    ...
    case("white ♔e1 ♙a7 ♚g8", "a7a8Q"),
    case("white ♔e1 ♖a1 ♚g8", "e1c1"),
    case("white ♔e1 ♖h1 ♚g8", "e1g1"),
    case("a2-a4 a7-a6 a4-a5 b7-b5", "a5b6"),
    ::trace //This leads to the arguments being printed in front of the test result.
    )]
    fn test_get_allowed_moves_contains(game_config: &str, expected_move_str: &str) {
        let expected_move = expected_move_str.parse::<BasicMove>().unwrap();
        let moves = get_allowed_moves(game_config);
    ...
    case("white ♔e1 ♙a7 ♚g8", "a7a8"),
    case("white ♔e1 ♙a7 ♚g8", "a7a8R"),
    case("white ♔e1 ♙a7 ♚g8", "a7a8K"),
    case("white ♔e1 ♙a7 ♚g8", "a7a8B"),
    ::trace //This leads to the arguments being printed in front of the test result.
    )]
    fn test_get_allowed_moves_doesnt_contain(game_config: &str, unexpected_move_str: &str) {
        let unexpected_move = unexpected_move_str.parse::<BasicMove>().unwrap();
        let moves = get_allowed_moves(game_config);
